...
/*
  PhotoBeam — Capacitor-native build
  ----------------------------------
  Storage model:
    - Preferences (key "photobeam-v3") holds ONLY metadata:
        { albums: [{id,name,ids}], imported: [{id,t,file,grp}] }
    - Image bytes live on the filesystem at Directory.Data/photos/<id>.jpg,
      downscaled to ≤1200px JPEG at import time.
    - At runtime, file paths resolve to displayable URLs:
        native → Filesystem.getUri + Capacitor.convertFileSrc
        web    → Filesystem.readFile → data URL (dev fallback)
    - Legacy v2 records that embedded base64 in metadata are migrated to
      files on first load, then re-saved path-only.

  Import:
    - Native: Camera.pickImages (PHPicker — no permission prompt on iOS 14+,
      but keep NSPhotoLibraryUsageDescription in Info.plist per Capacitor docs).
    - Web: hidden <input type="file"> fallback so browser dev still works.

  TV discovery remains simulated — real AirPlay needs a native plugin later.
*/
...
const STORE_KEY = "photobeam-v3";
const LEGACY_KEY = "photobeam-v2";
...
// ============================================================
// Image pipeline: downscale → write to Filesystem → resolve src
// ============================================================

// any displayable src (webPath, object URL, data URL) → raw base64 JPEG ≤ max px
function srcToJpegBase64(src, max = 1200, quality = 0.8) {
...
async function writePhotoFile(id, base64) {
...
// path on Directory.Data → something an <img src> can render
async function pathToDisplaySrc(path) {
  if (Capacitor.isNativePlatform()) {
    const { uri } = await Filesystem.getUri({ path, directory: Directory.Data });
    return Capacitor.convertFileSrc(uri);
  }
  // web dev fallback: Filesystem-on-web is IndexedDB; read back as data URL
...
function Playing({ source, tvName, lookup, onStop }) {
  ...
  return (
    <div className="flex-1 flex flex-col fade">
      <header className="px-5 pt-6 flex items-center gap-2">
...
export default function App() {
  ...
  const [imported, setImported] = useState([]); // user photos {id,t,file,grp} — path-only metadata
  const [srcMap, setSrcMap] = useState({}); // id → displayable URL, runtime only
  ...
  const lookup = useCallback(
    (id) => {
      const u = imported.find((p) => p.id === id);
      if (u) return { ...u, img: srcMap[id] };
      return SEED_PHOTOS.find((p) => p.id === id);
    },
    [imported, srcMap]
  );
  const allPhotos = [...imported.map((p) => ({ ...p, img: srcMap[p.id] })), ...SEED_PHOTOS];
  const GROUPS = [...new Set(allPhotos.map((p) => p.grp))];

  // resolve display URLs for any imported photos missing one
  useEffect(() => {
    const missing = imported.filter((p) => p.file && !srcMap[p.id]);
    ...
    (async () => {
      const entries = await Promise.all(
        missing.map(async (p) => {
          try {
            return [p.id, await pathToDisplaySrc(p.file)];
          } catch {
            return [p.id, null];
          }
        })
      );
      if (alive) setSrcMap((m) => ({ ...m, ...Object.fromEntries(entries) }));
    })();
    return () => { alive = false; };
  }, [imported]); // eslint-disable-line react-hooks/exhaustive-deps

  // ----- load persisted state (with v2 → v3 migration) -----
  useEffect(() => {
    (async () => {
      try {
        const r = await Preferences.get({ key: STORE_KEY });
        if (r.value) {
          const d = JSON.parse(r.value);
          if (Array.isArray(d.albums)) setAlbums(d.albums);
          if (Array.isArray(d.imported)) setImported(d.imported);
        } else {
          // migrate legacy v2 data that embedded base64 in metadata
          const old = await Preferences.get({ key: LEGACY_KEY });
          if (old.value) {
            const d = JSON.parse(old.value);
            const migrated = [];
            for (const p of d.imported || []) {
              if (p.img?.startsWith("data:")) {
                try {
                  const file = await writePhotoFile(p.id, p.img.split(",")[1]);
                  migrated.push({ id: p.id, t: p.t, file, grp: p.grp });
                } catch { /* skip */ }
              }
            }
            if (Array.isArray(d.albums)) setAlbums(d.albums);
  ...
  // ----- persist metadata only (debounced) -----
  useEffect(() => {
    if (loading) return;
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        await Preferences.set({
          key: STORE_KEY,
          value: JSON.stringify({ albums, imported }),
        });
      } catch {
  ...
  // ----- import: native PHPicker, web file-input fallback -----
  const addPhotos = async (items) => {
    // items: [{src, name}] where src is webPath/objectURL
    setImporting(true);
    const added = [];
    for (const it of items) {
      try {
        const id = `u${Date.now()}-${Math.random().toString(36).slice(2, 7)}`;
        const b64 = await srcToJpegBase64(it.src);
        const file = await writePhotoFile(id, b64);
        added.push({ id, t: it.name || "Imported photo", file, grp: "Your imports" });
        if (it.revoke) URL.revokeObjectURL(it.src);
      } catch {
    ...
    else setToast("Couldn't import those photos.");
  };

  const startImport = async () => {
    if (Capacitor.isNativePlatform()) {
      try {
        const res = await Camera.pickImages({ quality: 90, limit: 12 });
        if (!res.photos?.length) return;
        await addPhotos(
          res.photos.map((ph, idx) => ({ src: ph.webPath, name: `Import ${idx + 1}` }))
        );
      } catch {
  ...
  const onFiles = async (e) => {
    ...
    await addPhotos(
      files.map((f) => ({ src: URL.createObjectURL(f), name: f.name.replace(/\.[^.]+$/, ""), revoke: true }))
    );
  };

  // ----- delete imported photos (file + metadata + album refs) -----
  const selAllImported = sel.length > 0 && sel.every((id) => imported.some((p) => p.id === id));
  const deleteSelectedImports = async () => {
    const doomed = imported.filter((p) => sel.includes(p.id));
    for (const p of doomed) await deletePhotoFile(p.file);
    setImported((x) => x.filter((p) => !sel.includes(p.id)));
    setSrcMap((m) => {
      const n = { ...m };
      sel.forEach((id) => delete n[id]);
      return n;
    });
    setAlbums((a) =>
      a
        .map((al) => ({ ...al, ids: al.ids.filter((id) => !sel.includes(id)) }))
        .filter((al) => al.ids.length > 0)
    );
    setSel([]);
    setToast(`Deleted ${doomed.length} photo${doomed.length > 1 ? "s" : ""}`);
  };
      ...
      <div className="w-full max-w-md min-h-screen flex flex-col relative" style={{ background: C.bg }}>
        {screen === "playing" && source ? (
          ...
          <>
                      ...
                      <span className="grid grid-cols-2 gap-0.5 w-12 h-12 rounded-xl overflow-hidden shrink-0">
                        {a.ids.slice(0, 4).map((id) => {
                          ...
                          return p?.img ? (
                            ...
                            <span key={id} style={{ background: p?.g ? `linear-gradient(135deg, ${p.g[0]}, ${p.g[1]})` : C.card2 }} />
                          );
                        })}
                      </span>
              ...
              >
                ...
                {selAllImported && (
                  <button
                    ...
                    aria-label="Delete imported photos"
                    className="p-3 rounded-xl"
                    style={{ background: "rgba(248,113,113,.12)", color: C.red }}
                  >
                    <Trash2 size={18} />
                  </button>
                )}
            ...
            {/* tab bar */}
            <nav
              className="absolute bottom-0 inset-x-0 z-10 flex justify-around px-6 pt-3 pb-6"
              style={{ background: "rgba(10,12,19,.9)", backdropFilter: "blur(14px)", borderTop: `1px solid ${C.line}` }}
            >
              {[
